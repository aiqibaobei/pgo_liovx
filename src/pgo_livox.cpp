...
shared_mutex mtxKF; //for keyfram push and pop
mutex mtxICP;
...
void rosPubAndSubInit(ros::NodeHandle &n) {
  ...
  PointCloudBodySub = n.subscribe<sensor_msgs::PointCloud2>("/cloud_registered_body", 10, OriginPointCloudCB);
  RtkSub = n.subscribe<sensor_msgs::NavSatFix>("/rtk_data", 100, RtkCB);
  //aft optimize
  OdomAftOptPub = n.advertise<nav_msgs::Odometry>("/Odometry_aft_optimize", 100);
  CloudAftOptPub = n.advertise<sensor_msgs::PointCloud2>("/cloud_aft_optimize", 10);
  PathAftOptPub = n.advertise<nav_msgs::Path>("/path_aft_optimize", 100);
  //show loop and submap location in the whole map
  LoopCurScanPub = n.advertise<sensor_msgs::PointCloud2>("/loop_cur_scan", 100);
  LoopHistSubMapPub = n.advertise<sensor_msgs::PointCloud2>("/loop_hist_submap", 100);
...
void mergeCloud(const int index, const int num) {
  ...
  shared_lock<shared_mutex> lock(mtxKF);
...
void updatePoses(void) {
  ...
  unique_lock<shared_mutex> lock(mtxKF);
//#pragma omp parallel for num_threads(8)
...
void thFindLoop(void) {
  ...
  while (ros::ok()) {
    rate.sleep();
    if ((int) keyPoses.size() < pcMatcher->NUM_RECENT_FRAMES || prevLoopIndex == (int) keyPoses.size() - 1) {
      continue;
    }
...
void thIsam() {
  ...
  while (ros::ok()) {
    rate.sleep();
    if (optimizer->isGraphMade) {
      if (prevGraphSize == (int) optimizer->graph.size())
        continue;
...
int main(int argc, char **argv) {
  ...
  downSizerMap.setLeafSize(0.4, 0.4, 0.4);
